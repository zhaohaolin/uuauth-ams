...
public class CheckLoginController implements AuthConstants {
	...
	public void getLoginToken(HttpServletRequest req, HttpServletResponse resp)
			throws IOException {
		...
		if (null != cookies) {
			for (Cookie cookie : cookies) {
				if (logger.isDebugEnabled()) {
					logger.debug("cookie=[{}, {}, {}]",
							new Object[] { cookie.getName(), cookie.getValue(),
									cookie.getDomain() });
				}
				if (COOKIE_TOKEN.equals(cookie.getName())) {
					...
					// 如果已经登录，从auto跳回
					EmployeeInfo employee = (EmployeeInfo) sessionEmployeeService
							.get(token);
					if (logger.isDebugEnabled()) {
					...
					if (null != employee) {
						...
						String role = remoteAuthService.getUserRole(project,
								employee.getId());
						String autoLoginUrl = autoLoginService.getLoginUrl(
								employee, project, role, clientIp);
						jumpUrl = resp.encodeRedirectURL(autoLoginUrl + "&url="
								+ returnUrl);
